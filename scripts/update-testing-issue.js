...
const SOURCE_DIR = path.resolve('./packages/react-tna/src')
const GITHUB_REPO = `tuannafed/react-tna`
...
const hooks = getHooks()
  ...
  .map(hook => {
    const url = `https://github.com/${GITHUB_REPO}/tree/master/packages/react-tna/src/${hook.name}`
    return {
      ...hook,
...
3. Write tests for the specific hook in \`packages/react-tna/src/useExample/useExample.test.ts\`.
